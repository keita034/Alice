...
void CreateScatteringComputeMaterial(MaterialManager* manager_,IAdapter* adapter_)
{
	std::unique_ptr<ComputeMaterial> lMaterial = std::make_unique<ComputeMaterial>();
	ID3D12Device* lDevice = adapter_->GetDevice()->Get();

	//シェーダの読み込み
	lMaterial->computeShader = CreateUniqueShader("Resources/Shaders/AliceParticle/Scattering/Scattering.hlsl","main","cs_5_0",IShader::ShaderType::CS);
	...
	manager_->AddMaterial(lMaterial,"ComputScatteringGPUParticle");
}
