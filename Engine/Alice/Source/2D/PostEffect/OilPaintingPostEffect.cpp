...
void OilPaintingPostEffect::Initialize()
{
	if (needsInit)
	{
		...
		material->vertexShader->Create("Resources/Shaders/2D/PostEffect/OilPaintingVS.hlsl");

		//ピクセルシェーダの読み込み
		material->pixelShader = std::make_unique<Shader>();
		material->pixelShader->Create("Resources/Shaders/2D/PostEffect/OilPaintingPS.hlsl", "main", "ps_5_0");
...
void OilPaintingPostEffect::PostUpdate(RenderTarget* mainRenderTarget)
{
...
const std::string& OilPaintingPostEffect::GetType()
{
	return type;
}

OilPaintingPostEffect::OilPaintingPostEffect()
{
}

void OilPaintingPostEffect::Draw(RenderTarget* mainRenderTarget)
{
...
void OilPaintingPostEffect::MainRenderTargetDraw(RenderTarget* mainRenderTarget)
{
