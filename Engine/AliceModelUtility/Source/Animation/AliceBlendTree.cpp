...
bool AliceBlendTree::CoercionInsertAnimation(uint32_t handle_,bool complement_,float frame_)
{
	if ( isInsert )
	{
		nowAnimation = insertAnimation->GetMotionData();
	}

	insertAnimation->SetMotion(handle_);
	frame = frame_;
	isInsert = coercion = true;
	insertAnimationPhase = BEFORE;
	insertAnimationOneThirdFrame = insertAnimation->GetAnimeMaxflame() / 3.0f;
	complement = complement_;
...
const ReturnMotionNode* AliceBlendTree::GetMotion(const std::string& nodeName)
{
	...
	else
	{
		if ( insertAnimationPhase == BEFORE )
		{
			...
			{
				auto lMotionItr = find_if(nowAnimation->returnChannels.begin(),nowAnimation->returnChannels.end(),[ & ] (std::pair<const std::string,ReturnMotionNode> motion)
				{
					return motion.second.name == nodeName;
				});

				if ( lMotionItr == nowAnimation->returnChannels.end() )
				{
					lAnimation = zeroReturnMotionNode;
				}
				else
				{
					lAnimation = lMotionItr._Ptr->_Myval.second;
				}
