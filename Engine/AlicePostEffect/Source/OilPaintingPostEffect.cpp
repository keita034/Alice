...
void OilPaintingPostEffect::Initialize()
{
	if (needsInit)
	{
		...
		//頂点シェーダの読み込み
		material->vertexShader = CreateUniqueShader("Resources/Shaders/AlicePostEffect/OilPainting/OilPaintingVS.hlsl");

		//ピクセルシェーダの読み込み
		material->pixelShader = CreateUniqueShader("Resources/Shaders/AlicePostEffect/OilPainting/OilPaintingPS.hlsl", "main", "ps_5_0",IShader::ShaderType::PS);
...
void OilPaintingPostEffect::PostUpdate(RenderTarget* mainRenderTarget_)
{
...
void OilPaintingPostEffect::Draw(RenderTarget* mainRenderTarget_)
{
	ID3D12GraphicsCommandList* lCmdList = sCmdList->GetGraphicCommandList();
...
void OilPaintingPostEffect::MainRenderTargetDraw(RenderTarget* mainRenderTarget_)
{
	ID3D12GraphicsCommandList* lCmdList = sCmdList->GetGraphicCommandList();
