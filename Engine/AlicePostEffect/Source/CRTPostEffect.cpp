...
CRTPostEffect* CRTPostEffect::SGetInstance()
{
	static CRTPostEffect lInstance;
	return &lInstance;
}

void CRTPostEffect::Initialize()
{
	if (needsInit)
	{
		...
		//頂点シェーダの読み込み
		material->vertexShader = CreateUniqueShader("Resources/Shaders/AlicePostEffect/CRT/CRTVS.hlsl");

		//ピクセルシェーダの読み込み
		material->pixelShader = CreateUniqueShader("Resources/Shaders/AlicePostEffect/CRT/CRTPS.hlsl", "main", "ps_5_0", IShader::ShaderType::PS);
		...
		constantBuffer = CreateUniqueConstantBuffer(sizeof(float));
		constantBuffer->Update(&time);

		type = "CRT";
	}
}

void CRTPostEffect::PostUpdate(RenderTarget* mainRenderTarget_)
{
	time += incrementNum;
...
const std::string& CRTPostEffect::GetType()
{
	return type;
}

void CRTPostEffect::SetIncrementNum(float num)
{
	incrementNum = num;
}

void CRTPostEffect::Draw(RenderTarget* mainRenderTarget_)
{
	ID3D12GraphicsCommandList* lCmdList = sCmdList->GetGraphicCommandList();
...
void CRTPostEffect::MainRenderTargetDraw(RenderTarget* mainRenderTarget_)
{
	ID3D12GraphicsCommandList* lCmdList = sCmdList->GetGraphicCommandList();
