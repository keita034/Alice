...
{
	static DistortionPostEffect lInstance;
	return &lInstance;
}

void DistortionPostEffect::Initialize()
{
	if (needsInit)
	{
		...
		//頂点シェーダの読み込み
		material->vertexShader = CreateUniqueShader("Resources/Shaders/AlicePostEffect/Distortion/DistortionVS.hlsl");

		//ピクセルシェーダの読み込み
		material->pixelShader = CreateUniqueShader("Resources/Shaders/AlicePostEffect/Distortion/DistortionPS.hlsl", "main", "ps_5_0", IShader::ShaderType::PS);
		...
		constantBufferMap.center = { 0.5f,0.5f };
		constantBufferMap.power = 0.5f;
...
void DistortionPostEffect::PostUpdate(RenderTarget * mainRenderTarget_)
{
	Draw(mainRenderTarget_);
	MainRenderTargetDraw(mainRenderTarget_);
}

const std::string& DistortionPostEffect::GetType()
{
	return type;
}

void DistortionPostEffect::Draw(RenderTarget * mainRenderTarget_)
{
	ID3D12GraphicsCommandList* lCmdList = sCmdList->GetGraphicCommandList();
...
void DistortionPostEffect::MainRenderTargetDraw(RenderTarget * mainRenderTarget_)
{
	ID3D12GraphicsCommandList* lCmdList = sCmdList->GetGraphicCommandList();
