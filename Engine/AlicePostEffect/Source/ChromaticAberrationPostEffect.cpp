...
void ChromaticAberrationPostEffect::Initialize()
{
	if (needsInit)
	{
		...
		//頂点シェーダの読み込み
		material->vertexShader = CreateUniqueShader("Resources/Shaders/AlicePostEffect/ChromaticAberration/ChromaticAberrationVS.hlsl");

		//ピクセルシェーダの読み込み
		material->pixelShader = CreateUniqueShader("Resources/Shaders/AlicePostEffect/ChromaticAberration/ChromaticAberrationPS.hlsl", "main", "ps_5_0", IShader::ShaderType::PS);
		...
		constantBuffer = CreateUniqueConstantBuffer(sizeof(Offset));
...
void ChromaticAberrationPostEffect::PostUpdate(RenderTarget* mainRenderTarget_)
{
...
void ChromaticAberrationPostEffect::Draw(RenderTarget* mainRenderTarget_)
{
	ID3D12GraphicsCommandList* lCmdList = sCmdList->GetGraphicCommandList();
...
void ChromaticAberrationPostEffect::MainRenderTargetDraw(RenderTarget* mainRenderTarget_)
{
	ID3D12GraphicsCommandList* lCmdList = sCmdList->GetGraphicCommandList();
