...
{
	static MonochromePostEffect lInstance;
	return &lInstance;
}

void MonochromePostEffect::Initialize()
{
	if (needsInit)
	{
		...
		//頂点シェーダの読み込み
		material->vertexShader = CreateUniqueShader("Resources/Shaders/AlicePostEffect/Monochrome/MonochromeVS.hlsl");

		//ピクセルシェーダの読み込み
		material->pixelShader = CreateUniqueShader("Resources/Shaders/AlicePostEffect/Monochrome/MonochromePS.hlsl", "main", "ps_5_0", IShader::ShaderType::PS);
...
void MonochromePostEffect::PostUpdate(RenderTarget * mainRenderTarget_)
{
	Draw(mainRenderTarget_);
	MainRenderTargetDraw(mainRenderTarget_);
}

const std::string& MonochromePostEffect::GetType()
{
	return type;
}

void MonochromePostEffect::Draw(RenderTarget * mainRenderTarget_)
{
	ID3D12GraphicsCommandList* lCmdList = sCmdList->GetGraphicCommandList();
...
void MonochromePostEffect::MainRenderTargetDraw(RenderTarget * mainRenderTarget_)
{
	ID3D12GraphicsCommandList* lCmdList = sCmdList->GetGraphicCommandList();
...
MonochromePostEffect::MonochromePostEffect()
{
}
