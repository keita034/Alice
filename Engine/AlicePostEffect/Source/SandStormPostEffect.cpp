...
{
	static SandStormPostEffect lInstance;
	return &lInstance;
}

void SandStormPostEffect::Initialize()
{
	if (needsInit)
	{
		...
		//頂点シェーダの読み込み
		material->vertexShader = CreateUniqueShader("Resources/Shaders/AlicePostEffect/SandStorm/SandStormVS.hlsl");

		//ピクセルシェーダの読み込み
		material->pixelShader = CreateUniqueShader("Resources/Shaders/AlicePostEffect/SandStorm/SandStormPS.hlsl", "main", "ps_5_0", IShader::ShaderType::PS);
...
void SandStormPostEffect::PostUpdate(RenderTarget* mainRenderTarget_)
{
	time++;
	constantBuffer->Update(&time);
	if (time >= TIME_MAX)
...
const std::string& SandStormPostEffect::GetType()
{
	return type;
}

void SandStormPostEffect::Draw(RenderTarget* mainRenderTarget_)
{
	ID3D12GraphicsCommandList* lCmdList = sCmdList->GetGraphicCommandList();
...
void SandStormPostEffect::MainRenderTargetDraw(RenderTarget* mainRenderTarget_)
{
	ID3D12GraphicsCommandList* lCmdList = sCmdList->GetGraphicCommandList();
...
SandStormPostEffect::SandStormPostEffect()
{
}