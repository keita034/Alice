...
void GaussianYBlurPostEffect::Initialize()
{
	if (needsInit)
	{
		...
		//頂点シェーダの読み込み
		material->vertexShader = CreateUniqueShader("Resources/Shaders/AlicePostEffect/GaussianBlur/GaussianYBlurVS.hlsl");

		//ピクセルシェーダの読み込み
		material->pixelShader = CreateUniqueShader("Resources/Shaders/AlicePostEffect/GaussianBlur/GaussianBlurPS.hlsl", "main", "ps_5_0");
