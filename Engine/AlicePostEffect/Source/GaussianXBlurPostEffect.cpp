...
void GaussianXBlurPostEffect::Initialize()
{
	if (needsInit)
	{
		...
		width = static_cast<float>(sWindowsApp->GetWindowSize().width / 2);
		...
		//頂点シェーダの読み込み
		material->vertexShader = CreateUniqueShader("Resources/Shaders/AlicePostEffect/GaussianBlur/GaussianXBlurVS.hlsl");

		//ピクセルシェーダの読み込み
		material->pixelShader = CreateUniqueShader("Resources/Shaders/AlicePostEffect/GaussianBlur/GaussianBlurPS.hlsl", "main", "ps_5_0",IShader::ShaderType::PS);
		...
		{
			//レンダーターゲットの生成
			std::unique_ptr<IRenderTargetBuffer> lBuff = CreateUniqueRenderTargetBuffer(static_cast<uint32_t>(width), static_cast<uint32_t>(height), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
			...
			{//SRV作成
				...
				lHandle.ptr = sSrvHeap->CreateSRV(lSrvDesc, lBuff->GetTexture());
				handles.push_back(lHandle);
			}

			renderTargetBuffers.push_back(std::move(lBuff));
		}

		{
			//デプスステンシルの生成
			std::unique_ptr<IDepthStencilBuffer> lBuff = CreateUniqueDepthStencilBuffer(static_cast<uint32_t>(width), static_cast<uint32_t>(height), DXGI_FORMAT_D32_FLOAT);
			depthStencilBuffers.push_back(std::move(lBuff));
		}
...
void GaussianXBlurPostEffect::PostUpdate(RenderTarget* mainRenderTarget_)
{
...
void GaussianXBlurPostEffect::SetWeight(const std::array<float, 8>& weight_)
{
	weight = weight_;
...
void GaussianXBlurPostEffect::Draw(RenderTarget* mainRenderTarget_)
{
	ID3D12GraphicsCommandList* lCmdList = sCmdList->GetGraphicCommandList();
	...
	lCmdList->SetGraphicsRootConstantBufferView(1, weightBuff->GetAddress());
...
void GaussianXBlurPostEffect::MainRenderTargetDraw(RenderTarget* mainRenderTarget_)
{
	ID3D12GraphicsCommandList* lCmdList = sCmdList->GetGraphicCommandList();
	...
	CD3DX12_VIEWPORT lViewPort = CD3DX12_VIEWPORT(0.0f, 0.0f, width * 2, height);
	lCmdList->RSSetViewports(1, &lViewPort);

	CD3DX12_RECT lRect = CD3DX12_RECT(0, 0, static_cast<LONG>(width * 2), static_cast<LONG>(height));
	lCmdList->RSSetScissorRects(1, &lRect);
