...
struct PlayerGreatWeaponUsData
{
	AliceMathF::Vector3 velocity;
};

class PlayerGreatWeapon : public GameObject
{
private:

	AliceModel::AnimationTransform animationTransform;
	PlayerGreatWeaponUsData usData;
...
public:
	...
	virtual void Update(const AliceMathF::Matrix4& mat_);
...
private:
	...
	//コピーコンストラクタ・代入演算子削除
	PlayerGreatWeapon& operator=(const PlayerGreatWeapon&) = delete;
	PlayerGreatWeapon(const PlayerGreatWeapon&) = delete;
};
