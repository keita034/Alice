...
void ChromaticAberrationPostEffect::Initialize()
{
	if (needsInit)
	{
		...
		material->vertexShader->Create("Resources/Shaders/2D/PostEffect/ChromaticAberrationVS.hlsl");

		//ピクセルシェーダの読み込み
		material->pixelShader = std::make_unique<Shader>();
		material->pixelShader->Create("Resources/Shaders/2D/PostEffect/ChromaticAberrationPS.hlsl", "main", "ps_5_0");
		...
		material->rootSignature->Add(RootSignature::RootType::CBV, 0);//t0
		...
		constantBuffer->Create(sizeof(Offset));
		constBuffMap = new Offset;
		constBuffMap->size = 0.025f;
		constantBuffer->Update(constBuffMap);

		type = "CHROMATICABERRATION";
	}
}

void ChromaticAberrationPostEffect::PostUpdate(RenderTarget* mainRenderTarget)
{
...
const std::string& ChromaticAberrationPostEffect::GetType()
{
	return type;
}

ChromaticAberrationPostEffect::ChromaticAberrationPostEffect()
{
}

void ChromaticAberrationPostEffect::Draw(RenderTarget* mainRenderTarget)
{
	...
	cmdList->SetGraphicsRootConstantBufferView(1, constantBuffer->GetAddress());
...
void ChromaticAberrationPostEffect::MainRenderTargetDraw(RenderTarget* mainRenderTarget)
{
